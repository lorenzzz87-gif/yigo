/* ============================================================
   ...
   ============================================================ */
// 实例命名空间：同域多公司隔离（A 家 SUVOO_NS 为空，键不变；B 家加前缀）
const _NS = (typeof window !== 'undefined' && window.SUVOO_NS) || '';
const CLOUD_CFG_KEY = _NS + 'suvoo_ims_cloud2';
const CLOUD_TOKEN_KEY = _NS + 'suvoo_cloud_token';
const CLOUD_EMAIL_KEY = _NS + 'suvoo_cloud_email';
const SYNC_SNAP_KEY = _NS + 'suvoo_ims_sync_snap';
const SYNC_PULL_TS_KEY = _NS + 'suvoo_ims_pull_ts';
const CLOUD_DEFAULTS = { url: (typeof window !== 'undefined' && window.SUVOO_API_URL) || 'https://suvoo-api.ydanstudio.workers.dev' };
// workers.dev 子域名迁移：旧 lorenzzz87 已失效 → 改指 ydanstudio
function migrateApiHost(url) {
  return String(url || '').replace('.lorenzzz87.workers.dev', '.ydanstudio.workers.dev');
}
...
function getCloudCfg() {
  try {
    ...
    if (raw) {
      ...
      if (c && c.url) {
        const fixed = migrateApiHost(c.url);
        if (fixed !== c.url) { c.url = fixed; saveCloudCfg(c); } // 就地迁移旧地址
        return c;
      }
    }
  } catch (e) { /* ignore */ }
